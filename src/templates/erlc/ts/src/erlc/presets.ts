...
function toAllowlist(value: unknown): (number | PlayerPermission)[] {
  if (!Array.isArray(value)) return [];
  return value.filter((entry): entry is number | PlayerPermission => typeof entry === 'number' || typeof entry === 'string');
}
 ...
 */
export function registerPresets(client: Client): void {
  const p = erlcPresets;
  ...
  if (p.banCommand.enabled) {
    ...
    client.on(ERLCEvents.command, banCommand(
      commands,
      action,
      Boolean(p.banCommand.options.startsWith),
      toAllowlist(p.banCommand.options.allowlist),
    ));
  }

  if (p.banVehicles.enabled) {
    client.on(ERLCEvents.vehicleAdd, banVehicles(
      toArray(p.banVehicles.options.vehicles) as (Vehicles | string)[],
      VehiclePunishments.warnThenKick(Number(p.banVehicles.options.delay ?? 15)),
      toAllowlist(p.banVehicles.options.allowlist),
    ));
  }

  if (p.banLiveries.enabled) {
    client.on(ERLCEvents.vehicleAdd, banLiveries(
      toArray(p.banLiveries.options.liveries),
      LiveryPunishments.warnThenKick(Number(p.banLiveries.options.delay ?? 15)),
      toAllowlist(p.banLiveries.options.allowlist),
    ));
  }

  if (p.rdm.enabled) {
    const action = String(p.rdm.options.action ?? 'kick') === 'ban'
      ? (log: KillLog) => log.killer.ban()
      : (log: KillLog) => log.killer.kick();
    client.on(ERLCEvents.kill, RDM(
      action,
      toAllowlist(p.rdm.options.allowlist),
      Number(p.rdm.options.minKills ?? 4),
      Number(p.rdm.options.timespan ?? 30),
    ));
  }
